...
sex_interest <- "all"
# Plot phylogenies? Only choose TRUE if want to plot actual phylogenies - otherwise choose FALSE to save time
plot_phylo <- FALSE
...
## Calculate within-group vs between-group variance (ANOVA) ----

within_group_diversity <- as.data.frame(within_group_diversity)
# first check assumptions
hist(within_group_diversity[within_group_diversity$sex == "All", "group_metric"], n = 30)
library(ggfortify)
mod_all <- lm(log(group_metric) ~ taxon_subgroup, data = subset(within_group_diversity, sex == "All"))
autoplot(mod_all)
mod_m <- lm(log(group_metric) ~ taxon_subgroup, data = subset(within_group_diversity, sex == "M"))
autoplot(mod_m)
mod_f <- lm(log(group_metric) ~ taxon_subgroup, data = subset(within_group_diversity, sex == "F"))
autoplot(mod_f)
# ----> need to log-transform to satisfy normality assumption

# all specimens
all_anova <- aov(log(group_metric) ~ taxon_subgroup, data = subset(within_group_diversity, sex == "All"))
summary(all_anova)
# males
m_anova <- aov(log(group_metric) ~ taxon_subgroup, data = subset(within_group_diversity, sex == "M"))
summary(m_anova)
# females
f_anova <- aov(log(group_metric) ~ taxon_subgroup, data = subset(within_group_diversity, sex == "F"))
summary(f_anova)

# check for between-sex variance as well
mvf_anova <- aov(log(group_metric) ~ sex, data = subset(within_group_diversity, sex != "All"))
summary(mvf_anova)

# calculate within-vs-between group variances
calc_variance_part <- function(anova_mod){
  ss_between <- summary(anova_mod)[[1]]$`Sum Sq`[1]
  ss_within <- summary(anova_mod)[[1]]$`Sum Sq`[2]
  ss_total <- ss_between + ss_within
  eta_squared <- ss_between / ss_total
  between_group <- ss_between / ss_total * 100
  within_group <- ss_within / ss_total * 100
  print(paste0("eta squared = ", round(eta_squared, 2), ", between-group variance = ", round(between_group, 2), "%, within-group variance = ", round(within_group, 2), "%"))
  return(c(eta_squared, between_group, within_group))
}
# calculate proportion of variance explained by group membership
# all specimens
calc_variance_part(all_anova)
# males
calc_variance_part(m_anova)
# females
calc_variance_part(f_anova)
# males vs females
calc_variance_part(mvf_anova)

# now do MANOVA with the full PCA data
m_pca_dat <- pca_dat[pca_dat$sex == "M", ]
pcs <- cbind(m_pca_dat$PC1, m_pca_dat$PC3, m_pca_dat$PC4, m_pca_dat$PC5, m_pca_dat$PC6, m_pca_dat$PC7, m_pca_dat$PC8, m_pca_dat$PC9, m_pca_dat$PC10, m_pca_dat$PC11, m_pca_dat$PC12, m_pca_dat$PC13, m_pca_dat$PC14, m_pca_dat$PC15, m_pca_dat$PC16, m_pca_dat$PC17, m_pca_dat$PC18, m_pca_dat$PC19, m_pca_dat$PC20, m_pca_dat$PC21, m_pca_dat$PC22, m_pca_dat$PC23, m_pca_dat$PC24, m_pca_dat$PC25, m_pca_dat$PC26, m_pca_dat$PC27, m_pca_dat$PC28, m_pca_dat$PC29, m_pca_dat$PC30)
m_manova <- manova(pcs ~ taxon_subgroup, data = m_pca_dat)
summary(m_manova)
effectsize::eta_squared(m_manova)
f_pca_dat <- pca_dat[pca_dat$sex == "F", ]
pcs <- cbind(f_pca_dat$PC1, f_pca_dat$PC3, f_pca_dat$PC4, f_pca_dat$PC5, f_pca_dat$PC6, f_pca_dat$PC7, f_pca_dat$PC8, f_pca_dat$PC9, f_pca_dat$PC10, f_pca_dat$PC11, f_pca_dat$PC12, f_pca_dat$PC13, f_pca_dat$PC14, f_pca_dat$PC15, f_pca_dat$PC16, f_pca_dat$PC17, f_pca_dat$PC18, f_pca_dat$PC19, f_pca_dat$PC20, f_pca_dat$PC21, f_pca_dat$PC22, f_pca_dat$PC23, f_pca_dat$PC24, f_pca_dat$PC25, f_pca_dat$PC26, f_pca_dat$PC27, f_pca_dat$PC28, f_pca_dat$PC29, f_pca_dat$PC30)
f_manova <- manova(pcs ~ taxon_subgroup, data = f_pca_dat)
summary(f_manova)
effectsize::eta_squared(f_manova)
