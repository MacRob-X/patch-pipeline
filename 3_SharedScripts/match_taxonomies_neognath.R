...
# remove non-neognaths from patch pixel values
...
# NR species AND are 1_bl_to_1_bt mapping AND do not appear in the IUCN 2023 data - these will not have an 
...
problem_species <- avonet_crosswalk_checked[(avonet_crosswalk_checked$species_birdlife %in% bl_5$species_5[!(bl_5$sis_rec_id %in% bl_81$sis_rec_id[bl_81$bl_tax_treat != "NR"]) & !(bl_5$species_5 %in% iucn_aves$species_birdlife)]) & avonet_crosswalk_checked$match_type == "1_bl_to_1_bt", ]
...
# ---------------------------------------- 2025-07-21
# We will skip this entire section, as we're rematching from scratch and we want to 
# include all BirdTree species
# 
...
# 
# # UPDATE 2025-07-18 ------------------------------------#
# # Get the (Neognath) species for which we have new data
...
# UPDATE 21/07/2025 - this is also true for two extra species, Caprimulgus centralasicus
# and Tyto almae - have cross-checked sis_rec_id in BLv5 and BLv8.1 to confirm
...
## UPDATE 21/07/2025 - it differs for Caprimulgus centralasicus and Tyto almae as these 
## were previously classed as DD. For Tyto almae, it has been amalgamated into Tyto novaehollandae
## and Caprimulgus centralasicus has been amalgamated into C. europaeus (from HBW-BirdLife Checklistv8.1
## Taxonomic Notes). Since these have not become subspecies, we will continue to classify them
## as DD so they are removed from any downstream analysis
# we will assign their initial categories as DD and remove them from this dataframe
#  they will then be assigned a final category of DD later
avonet_iucn[avonet_iucn$species_birdlife == "Caprimulgus_centralasicus", "initial_category"] <- "DD"
avonet_iucn[avonet_iucn$species_birdlife == "Tyto_almae", "initial_category"] <- "DD"
na_init_iucn <- na_init_iucn[-which(na_init_iucn$species_birdlife == "Caprimulgus_centralasicus" | na_init_iucn$species_birdlife == "Tyto_almae"), ]
# In any case we don't have both male and female colour data for these species, so they will be excluded
# from downstream analysis anyway
...
# UPDATE 2025-08-08
# most of these are 'NA' because they're in BL but not BT - we can ignore these for the moment I think
# let's filter these NAs out for the moment
dupes <- dupes[!is.na(dupes)]
# END UPDATE 2025-08-08
...
iucn_2024_conservative %>%
  readr::write_csv(
    file = here::here(
    "4_SharedInputData", "neognath_iucn_2024_conservative.csv"
  ),
    col_names = TRUE
  )
...
iucn_2024_liberal %>%
  readr::write_csv(
    file = here::here(
      "4_SharedInputData", "neognath_iucn_2024_liberal.csv"
    ),
    col_names = TRUE
  )


# same for the nominate dataset
...
iucn_2024_nominate %>%
  readr::write_csv(
    file = here::here(
      "4_SharedInputData", "neognath_iucn_2024_nominate.csv"
    ),
    col_names = TRUE
  )
